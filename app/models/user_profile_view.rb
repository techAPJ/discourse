class UserProfileView < ActiveRecord::Base
  ...
  def self.profile_views_by_day(start_date, end_date, group_id=nil)
    ...
    if group_id
      ...
      profile_views = profile_views.joins("INNER JOIN group_users ON group_users.user_id = users.id") if group_id
      profile_views = profile_views.where("group_users.group_id = ?", group_id) if group_id
    end
