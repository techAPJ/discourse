...
class ImportScripts::Ning < ImportScripts::Base
  ...
  JSON_FILES_DIR = "/Users/techapj/Downloads/ben/ATAXIA"
  ...
  def initialize
    ...
    # @videos_json      = load_ning_json("ning-videos.json")
    # @notes_json      = load_ning_json("ning-notes.json")
    ...
    # begin
    #   create_admin(email: 'arpit.jalan@discourse.org', username: UserNameSuggester.suggest('techapj'))
    # rescue => e
  ...
  def import_categories
    ...
    # create_categories((["Blog", "Pages", "Photos", "Notes"] + @discussions_json.map { |d| d["category"] }).uniq.compact) do |name|
  ...
  #   import_topics(@groups_json, "Groups")
  # end
  ...
  def import_videos
    ...
    import_topics(@videos_json, "Videos")
  end

  def import_notes
    puts "", "Importing notes"
    import_topics(@notes_json, "Notes")
  end
