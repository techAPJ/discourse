...
class BulkImport::Vanilla < BulkImport::Base
  ...
  VANILLA_DB = "plex"
  TABLE_PREFIX = "GDN_"
  # ATTACHMENTS_BASE_DIR = "/home/techapj/Documents/plex_uploads"
  ATTACHMENTS_BASE_DIR = "/Users/techapj/Documents/plex_uploads"
  BATCH_SIZE = 1000
  CONVERT_HTML = true
  ...
  def import_attachments
    if ATTACHMENTS_BASE_DIR && File.exists?(ATTACHMENTS_BASE_DIR)
      ...
      # https://us.v-cdn.net/6025034/uploads/editor/bm/bfv1mcco62ze.jpg
      cdn_regex = /https:\/\/us.v-cdn.net\/6025034\/uploads\/(\S+\/(\w|-)+.\w+)/i
      # [attachment=10109:Screen Shot 2012-04-01 at 3.47.35 AM.png]
      attachment_regex = /\[attachment=(\d+):(.*?)\]/i

      puts Post.where("raw LIKE '%/us.v-cdn.net/%'").count
      puts Post.where("raw LIKE '%[attachment%'").count
      puts Post.where("raw LIKE '%/us.v-cdn.net/%' OR raw LIKE '%[attachment%'").count
...
# IMPORT=1 bundle exec ruby script/bulk_import/vanilla.rb
